...
THEME_REC *theme_load(const char *setname)
{
	...
	/* check home dir */
	fname = g_strdup_printf("%s/%s.theme", get_irssi_data_dir(), name);
...
static void theme_save(THEME_REC *theme, int save_all)
{
	...
	path = g_strdup_printf("%s/%s", get_irssi_data_dir(), basename);
...
void themes_reload(void)
{
	...
	if (current_theme == NULL) {
		fname = g_strdup_printf("%s/default.theme", get_irssi_data_dir());
